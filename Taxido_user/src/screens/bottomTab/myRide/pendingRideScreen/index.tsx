...
export function PendingRideScreen() {
      ...
      .then((res: any) => {
        ...
        if (res?.ride_status?.slug == "cancelled") {
          ...
          if (rideId) {
            ...
            notificationHelper(
              "",
              translateData.rideCancelNotification,
              "error",
            );
          } else {
  ...
  // Echo real-time ride status listener
  useEffect(() => {
    ...
    const setupEchoListener = async () => {
      try {
        ...
        echo
          .private(`ride.${rideId}`)
          .listen(".ride.status_update", (data: any) => {
            const status = data?.status;
            if (status) {
              // Refresh ride data from API to keep Redux state in sync
              dispatch(allRide({ ride_id: rideId }))
                .unwrap()
                .catch((error: any) => {
                  console.error("[PendingRide] allRide refresh error:", error);
                });

              if (status === "started") {
                // Use Echo event data instead of Firebase rideDoc
                setLoader(false);
                // @ts-ignore - Navigation type issue
                navigation.navigate("RideActive", { activeRideOTP: data });
  ...
  const gotoOtpRide = () => {
    try {
      ...
      if (!rideId) {
        ...
        notificationHelper(
          "",
          translateData?.somethingWrong || "Ride ID missing",
          "error",
        );
        return;
      }
      ...
      if (item?.service_category?.service_category_type === "package") {
        navigate("PaymentRental", { rideId });
      } else {
        navigate("Payment", { rideId });
      }
    } catch (e) {
      console.error("[StartRide] Navigation error:", e);
      notificationHelper(
        "",
        translateData?.somethingWrong || "Something went wrong",
        "error",
      );
    } finally {
      setLoader(false);
          ...
          >
            <View style={[styles.container]}>
              ...
              >
                ...
                >
                  <Image
                    source={isDark ? Images.rideDark : Images.map}
                    resizeMode="stretch"
                    style={{ height: windowHeight(80), width: "100%" }}
                  />
                </TouchableOpacity>
                <View
                  style={{
            ...
            {item?.service_category?.service_category_type !== "schedule" &&
              ...
              item?.service_category?.service_category_type !== "ride" && (
                ...
                >
                  ...
                  <View
                    style={[styless.viewRental, { alignItems: "flex-end" }]}
                  >
                    <Text
                      style={[
                        styless.startDateText,
            ...
            {item.ride_status.slug != "cancelled" && (
              <View
                style={[
                  styless.ride_statusView,
                  { backgroundColor: bgFullStyle },
                ]}
              >
                <View
                  style={[
                    styless.paymentMethodView,
                    {
                      backgroundColor: isDark
                        ? appColors.darkBorder
                        : appColors.lightGray,
                ...
                >
                  ...
                  />
                  <View
                    style={[styles.contain, { flexDirection: viewRTLStyle }]}
                  >
                    <Text
                      style={[styles.type, { color: appColors.regularText }]}
                    >
                      {translateData.paymentMethod}
                    </Text>
                    <Text
                ...
                <Image
                  ...
                  style={{
                    tintColor: isDark ? appColors.bgDark : appColors.lightGray,
                    width: "100%",
                    resizeMode: "stretch",
                  }}
                />
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                navigate("ChatScreen", {
                  from: "help",
                  riderId: self && self?.id,
                })
              }
              style={styless.needHelpView}
            >
              <Text
                style={[
                ...
                <View style={{ marginHorizontal: windowWidth(20) }}>
                  {item?.service?.service_type === "finddriver" ? (
                    <Button
                      ...
                      title={translateData.modelCancelText}
                      onPress={cancelRental}
                      loading={loader}
                    />
                  ) : (
                    <Button
            ...
            <View style={styless.ride_status}>
              ...
              {item?.ride_status?.slug == "completed" &&
                item?.payment_status == "COMPLETED" && (
                  <Button
                    title={translateData.viewInvoice}
                    onPress={viewInvoice}
                    loading={loader}
                  />
                )}
              ...
              <TouchableWithoutFeedback onPress={() => { }}>
                ...
                >
                  ...
                  <Image
                    ...
                    tintColor={
                      isDark ? appColors.darkBorder : appColors.whiteColor
                    }
                  />
                  <Text
                    style={[
