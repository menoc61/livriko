...
import { allRides, paymentsData, allRide } from "@src/api/store/actions";
...
export function PaymentRental() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const checkRideStatus = () => {
          dispatch(allRide({ ride_id: rideId }) as any)
            ...
            .catch((err: any) => {
              console.error("Failed to poll ride state:", err);
            });
        };
    ...
    return () => {
      ...
      if (echoInstance && rideId) {
        echoInstance.leave(`ride-status.${rideId}`);
      }
    };
  }, [rideId, dispatch]);
  ...
  useEffect(() => {
    ...
    getEchoInstance().then((echo: any) => {
      ...
      echo.join(`driver-notification.${driverId}`)
        .here((members: any[]) => {
        ...
        .joining((member: any) => {
          console.log("📍 [Pusher PaymentRental] Member Joined:", member);
        })
        .error((error: any) => {
          console.error("📍 [Pusher PaymentRental] Error Joining Presence Channel:", error);
        })
    ...
    return () => {
      isEchoActive = false;
      if (echoInstance) echoInstance.leave(`driver-notification.${driverId}`);
    };
  }, [rideDatas?.driver?.id]);
