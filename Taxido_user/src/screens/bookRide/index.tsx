  ...
  const handleRideAccepted = useCallback(
    async (rideData: any) => {
      ...
      if (rideData?.service_category?.service_category_type === "schedule") {
        ...
        setTimeout(() => {
          try {
            const cleanedData = rideData ? JSON.parse(JSON.stringify(rideData)) : null;
            ...
            notificationHelper("", "Navigation Failed: " + String(navError), "error");
          }
        }, 500);
      }
    },
  ...
  useEffect(() => {
    ...
    const setupStatusListener = async () => {
      ...
      echoInstance
        ...
        .error((err: any) => {
          notificationHelper("", "Subscription failed: " + (err?.message || "Check connection"), "error");
        })
        .listen(".bid.new", (e: any) => {
          ...
          if (newBid) {
            const formattedBid = {
              ...newBid,
              id: newBid.id || newBid.bid_id || newBid.driver_id || String(Date.now()),
            ...
            setDriverBidValue((prev: any[]) => {
              const exists = prev.some(b => b.id === formattedBid.id || (b.driver && b.driver.id === formattedBid.driver.id));
        ...
        >
          <View>
            ...
            >
              ...
              >
                {isExpanding ? translateData.searchDriver : "Choose a Vehicle"}
              </Text>
              {isExpanding && (
                <View
