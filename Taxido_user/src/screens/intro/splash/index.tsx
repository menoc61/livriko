...
import { resetRateLimit } from "@src/api/ratelimitter";
import { resetThrottler } from "@src/api/requestThrottler";
...
export function Splash() {
  ...
  useEffect(() => {
    const setDefaultLanguage = async () => {
      if (
        ...
        !settingData?.values?.general?.default_language
      )
        return;
  ...
  useEffect(() => {
    const updateSplashImage = async () => {
      const serverImage = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.cabbooking_values?.setting?.splash_screen_url
        : null;
      try {
  ...
  const proceedToNextScreen = useCallback(async () => {
    try {
      ...
      const requiredVersion = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.cabbooking_values?.setting?.app_version
        : null;
      const forceUpdate = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.cabbooking_values?.activation?.force_update == "1"
        : false;
