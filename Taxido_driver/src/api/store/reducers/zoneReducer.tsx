...
import { driverZone, rentalZone, currentZone, driversStatus } from '../action/zoneAction'
...
const zoneSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(driversStatus.pending, (state) => {
      state.loading = true;
    });
    builder.addCase(driversStatus.fulfilled, (state, action) => {
      state.driverStatus = action.payload;
      state.loading = false;
    });
    builder.addCase(driversStatus.rejected, (state) => {
      state.loading = false;
      state.success = false;
    });
