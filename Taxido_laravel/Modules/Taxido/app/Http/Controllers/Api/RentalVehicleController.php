...
class RentalVehicleController extends Controller
{
    ...
    public function filter($rentalVehicles, $request)
    {
        if (isset($request->start_time)) {
            ...
            if (!$endTime && $request->no_of_days) {
                $endTime = Carbon::parse($startTime)->addDays((int) $request->no_of_days)->toDateTimeString();
            }

            $rideRentalVehicleIds = RentalVehicle::whereHas('rides', function ($rides) use ($startTime, $endTime) {
                ...
                if ($endTime) {
                    $rides->where(function ($query) use ($startTime, $endTime) {
                        $query->where('start_time', '<=', $endTime)
                            ->where('end_time', '>=', $startTime);
                    });
                } else {
                    $rides->where('start_time', '<=', $startTime)
                        ->where('end_time', '>=', $startTime);
                }
            })->pluck('id')->toArray();
