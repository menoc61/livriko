...
class RideRequestResource extends BaseResource
{
  ...
  public function toArray(Request $request): array
  {
    ...
    if ($this->rider_id) {
      ...
      $rideRequest['rider'] = [
        'name' => $this->rider['name'] ?? ($riderData?->name ?? 'Guest - 2'),
    ...
    if ($this->driver) {
      $rideRequest['driver'] = [
        ...
        'vehicle_info' => [
          ...
          'vehicle_image_url' => $this->driver?->vehicle_info?->vehicle?->vehicle_image?->original_url,
          'vehicle_name' => $this->driver?->vehicle_info?->vehicle?->name,
        ],
      ];
    }

    if ($this->preferences && $this->preferences->count()) {
      $rideRequest['preferences'] = PreferenceResource::collection($this->preferences);
    }
