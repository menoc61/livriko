                                ...
                                <div id="find_driver_container"
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-2" for="price_per_type">
                                            {{ __('taxido::static.drivers.price_per_type') }}<span>*</span>
                                        </label>
                                        <div class="col-md-10 select-label-error">
                                            <select class="form-control select-2" id="price_per_type"
                                                ...
                                                <option value="per_km_charge"
                                                    {{ in_array('per_km_charge', old('price_per_type', @$driver?->price_per_type ?? [])) ? 'selected' : '' }}>
                                                    {{ __('taxido::static.drivers.per_km_charge') }}
                                                </option>
                                                <option value="per_hour_charge"
                                                    {{ in_array('per_hour_charge', old('price_per_type', @$driver?->price_per_type ?? [])) ? 'selected' : '' }}>
                                                    {{ __('taxido::static.drivers.per_hour_charge') }}
                                                </option>
                                                <option value="per_day_charge"
                                                    {{ in_array('per_day_charge', old('price_per_type', @$driver?->price_per_type ?? [])) ? 'selected' : '' }}>
                                                    {{ __('taxido::static.drivers.per_day_charge') }}
                                                </option>
                                            </select>
                                            @error('price_per_type')
                                                <span class="invalid-feedback d-block" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                            @enderror
    ...
    <script>
        ...
        (function($) {
            ...
            const ambulanceServiceId =
                {{ \Modules\Taxido\Models\Service::where('slug', 'ambulance')->first()->id ?? 0 }};
            const findDriverServiceId =
                {{ \Modules\Taxido\Models\Service::where('slug', 'find-driver')->first()->id ?? 0 }};

            const selectedVehicleId =
                "{{ old('vehicle_info.vehicle_type_id', @$driver?->vehicle_info?->vehicle_type_id) }}";

            // Check if find driver service is selected
            const isFindDriverService = {{ $isFindDriverService ? 'true' : 'false' }};
                    ...
                    success: (data) => {
                        ...
                        data.data.forEach(item => {
                            ...
                            @if (isset($driver) && $driver?->service_categories)
                                if ({{ json_encode($driver?->service_categories->pluck('id')->toArray()) }}
                                    .includes(item.id)) {
            ...
            function loadVehicles(serviceId, serviceCategoryId) {
                $.ajax({
                    url: "{{ route('vehicleType.index') }}?service_id=" + serviceId +
