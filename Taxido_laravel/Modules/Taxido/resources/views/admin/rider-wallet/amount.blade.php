    ...
    <div class="inside">
        ...
        <div class="wallet-detail">
            <div class="wallet-detail-content">
                ...
                @if (Auth::user()->hasRole(RoleEnum::ADMIN) || Auth::user()?->can('rider_wallet.credit') || Auth::user()?->can('rider_wallet.debit'))
                    <form action="{{ route('admin.rider-wallet.update.balance') }}" method="POST" id="rider-wallet"
                        class="wallet-form">
                        @csrf
...
@push('scripts')
    ...
    <script>
        (function($) {
            "use strict";
            const validator = $("#rider-wallet").validate({
                rules: {
                    'note': "required"
                }
            });
